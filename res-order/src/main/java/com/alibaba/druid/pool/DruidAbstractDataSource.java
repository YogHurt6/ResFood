...
public abstract class DruidAbstractDataSource extends WrapperAdapter implements DruidAbstractDataSourceMBean, DataSource, DataSourceProxy, Serializable {
    ...
    public static final int DEFAULT_TIME_CONNECT_TIMEOUT_MILLIS = 10000;
    public static final int DEFAULT_TIME_SOCKET_TIMEOUT_MILLIS = 10000;
...
//            } else {
    ...
    public Connection createPhysicalConnection(String url, Properties info) throws SQLException {
        Object conn;
        if (this.getProxyFilters().isEmpty()) {
    ...
    public DruidAbstractDataSource.PhysicalConnectionInfo createPhysicalConnection() throws SQLException {
        ...
        try {
            ...
            if (this.socketTimeout > 0 && !this.netTimeoutError) {
                try {
                    conn.setNetworkTimeout(this.netTimeoutExecutor, this.socketTimeout);
                } catch (AbstractMethodError | SQLFeatureNotSupportedException var30) {
                    this.netTimeoutError = true;
            ...
            this.setCreateError(var32);
            JdbcUtils.close(conn);
            throw var32;
        } catch (RuntimeException var33) {
            this.setCreateError(var33);
            JdbcUtils.close(conn);
            throw var33;
        } catch (Error var34) {
            createErrorCountUpdater.incrementAndGet(this);
            this.setCreateError(var34);
            JdbcUtils.close(conn);
            throw var34;
        } finally {
            if (var29) {
    ...
    public void initPhysicalConnection(Connection conn, Map<String, Object> variables, Map<String, Object> globalVariables) throws SQLException {
        ...
        if (!initSqls.isEmpty() || variables != null || globalVariables != null) {
            Statement stmt = null;
    ...
    class SynchronousExecutor implements Executor {
        ...
        public void execute(Runnable command) {
            try {
                ...
                DruidAbstractDataSource.this.netTimeoutError = true;
            } catch (Exception var4) {
                if (DruidAbstractDataSource.LOG.isDebugEnabled()) {
                    DruidAbstractDataSource.LOG.debug("failed to execute command " + command);
                }
            }
